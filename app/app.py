...
def load_model():
    ...
    # model_path = Path(__file__).parent / "model_joblib.joblib"
...
def get_decision_badge(probability: float) -> tuple:
    ...
    if probability < THRESHOLD_LOW:
        badge = '<span class="badge-approved">✅ Bajo Riesgo</span>'
        explanation = """
        **Recomendación: Bajo Riesgo**
        
        Estilo de vida saludable, parámetros metabólicos normales y pocos factores hereditarios relevantes.

        Pregnancies	0–3 (riesgo bajo por antecedentes)  
        Glucose	< 100 mg/dL  
        BloodPressure	< 80 mmHg  
        SkinThickness	10–30 mm (normal)  
        Insulin	< 100 μU/mL  
        BMI	< 25  
        ...
        explanation = """
        **Recomendación: RIESGO MODERADO**
        
        Paciente con factores de riesgo relevantes, estilo de vida menos saludable, o señales prediabéticas.

        Pregnancies	3–6  
        Glucose	100–125 mg/dL (prediabetes)  
        BloodPressure	80–89 mmHg  
        SkinThickness	30–40 mm  
        Insulin	100–150 μU/mL  
        BMI	25–29.9  
        DiabetesPedigreeFunction	0.4–0.8  
        Age	40–55 años
        """
        color = "warning"
    else:
        badge = '<span class="badge-rejected">❌ ALTO RIESGO</span>'
        explanation = """
        **Recomendación: ALTO RIESGO**
        
        Valores clínicamente peligrosos o altamente indicativos de diabetes, obesidad o resistencia a insulina.

        Pregnancies	> 6  
        Glucose	≥ 126 mg/dL (diabetes probable)  
